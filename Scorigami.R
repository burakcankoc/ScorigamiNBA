...
logall = nbastatR::game_logs(seasons = 1947:2020, result_types = "team", season_types = c("Regular Season", "Playoffs"))
...
logs %>% 
  ...
  ungroup() %>% 
  mutate(asd = cumsum(n)) %>% 
  ggplot(aes(x = yearSeason, y = asd)) +
  geom_point(size = 2) +
  theme_bw() +
  scale_x_continuous(breaks = seq(1950,2020,10), limits = c(1947,2020)) +
  #scale_y_continuous(breaks = s, limits = c(2,4))
  labs(title = "Cumulative Unique NBA Scores by Season",
       subtitle = "Regular Season and Playoff Scores, 1947-2020",
